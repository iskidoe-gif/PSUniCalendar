    ...
    <div class="min-h-screen">
        <main class="overflow-y-auto p-8">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-6">
                ...
                <form method="POST" action="{{ route('superadmin.venues.store') }}" class="flex gap-3 items-end">
                    @csrf
                    <div class="flex-1">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                <div class="mb-4 flex items-center justify-between">
                    ...
                    <span class="rounded-full bg-indigo-100 px-3 py-1 text-xs font-semibold text-indigo-700">{{ $venues->count() }} venues</span>
                </div>
                            ...
                            @forelse($venues as $venue)
                                ...
                                <tr>
                                    <td class="py-3 px-4 font-semibold text-gray-800">
                                        <form method="POST" action="{{ route('superadmin.venues.update', $venueId) }}" class="flex gap-2 items-center">
                                            @csrf
                                            @method('PUT')
                                            <input type="text" name="name" value="{{ $venueName }}" class="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500">
                                    ...
                                    <td class="py-3 px-4">{{ $eventCount }}</td>
                                    <td class="py-3 px-4">
                                        <div class="flex gap-2">
                                            <a href="{{ route('superadmin.venues.events', ['venue' => $venueDisplayName]) }}" class="px-3 py-1 bg-indigo-600 hover:bg-indigo-700 text-white rounded text-xs font-bold">View Events</a>
                                            <form method="POST" action="{{ route('superadmin.venues.destroy', $venueId) }}" onsubmit="return confirm('Delete this venue?');">
                                                @csrf
                                                @method('DELETE')
